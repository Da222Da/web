...
function createGeneralizationNode() {
  ...
  list.forEach((item, index) => {
    ...
    if (!cur.generalizationNode) {
      cur.generalizationNode = new MindMapNode_default({
        data: {
...
var Base2 = class {
  ...
  createNode(data2, parent, isRoot, layerIndex, index, ancestors) {
    ...
    if (data2 && data2._node && !this.renderer.reRender) {
      ...
      newNode = new MindMapNode_default({
        data: data2,
        uid: newUid,
        renderer: this.renderer,
        mindMap: this.mindMap,
...
var KeyCommand = class {
  ...
  removeShortcut(key, fn) {
    key.split(/\s*\|\s*/).forEach((item) => {
      if (this.shortcutMap[item]) {
        if (fn) {
          let index = this.shortcutMap[item].findIndex((f2) => {
            return f2 === fn;
          });
          if (index !== -1) {
